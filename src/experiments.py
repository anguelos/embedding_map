...
def get_map_bounds(query_fetaures, query_labels, retrieval_features,
                   retrieval_labels, metric="cosine", e=1e-20):
    dist_mat = cdist(query_fetaures, retrieval_features,
                     metric)  # *100).astype("int32")/100
    correct = (retrieval_labels[None, :] == query_labels[:, None]).astype(
        "float64")
    ...
    for dm in [optimist, pessimist]:
        ...
        correct_retrivals = correct[
            np.arange(dist_mat.shape[0])[:, np.newaxis], indexes]
        ...
        precision_at = correct_at / np.cumsum(np.ones_like(correct_retrivals),
                                              axis=1)
...
def plot_dist_colisions(query_fetaures, query_labels, retrieval_features,
                        retrieval_labels, metric="cosine", e=1e-10, quanta=None,
                        fname=None):
    plt.clf()
    ...
    correct = (retrieval_labels[idx[:, 1:]] == query_labels[:, None]).astype(
        "float")
...
def plot_phocnet_colisions():
    ...
    plot_dist_colisions(phoc_features[indexes], phoc_labels[indexes],
                        phoc_features[indexes], phoc_labels[indexes],
                        metric="cosine",
                        fname="_GW_PHOCNET_indicative_retrievals_cosine.pdf")
    plot_dist_colisions(phoc_features[indexes], phoc_labels[indexes],
                        phoc_features[indexes], phoc_labels[indexes],
                        metric="euclidean",
                        fname="_GW_PHOCNET_indicative_retrievals_euclidean.pdf")
    plot_dist_colisions(phoc_features[indexes], phoc_labels[indexes],
                        phoc_features[indexes], phoc_labels[indexes],
                        metric="cityblock",
                        fname="_GW_PHOCNET_indicative_retrievals_cityblock.pdf")
...
def plot_whitenoise_colisions():
    ...
    plot_dist_colisions(rnd_features[indexes], rnd_labels[indexes],
                        rnd_features[indexes], rnd_labels[indexes],
                        metric="cosine",
                        fname="_white_noise_embeddings_indicative_retrievals_cosine.pdf",
                        e=e)
    plot_dist_colisions(rnd_features[indexes], rnd_labels[indexes],
                        rnd_features[indexes], rnd_labels[indexes],
                        metric="euclidean",
                        fname="_white_noise_embeddings_indicative_retrievals_euclidean.pdf",
                        e=e)
    plot_dist_colisions(rnd_features[indexes], rnd_labels[indexes],
                        rnd_features[indexes], rnd_labels[indexes],
                        metric="cityblock",
                        fname="_white_noise_embeddings_indicative_retrievals_cityblock.pdf",
                        e=e)
